...
function resourceModel (sequelize, resourceName, options = {}) {
  const { valueBaseLength = 255 } = options
  ...
  const methods = {
    async ofCID (cid) {
      const Model = this
      return await Model.findOne({ where: { cid } })
    },
    async inbound (attributes, opt) {
      ...
      for (const [k, v]of Object.entries(attributes)) {
        ...
        if (needsExtra) {
          result.___extraValues[name] = serialized
          result[name] = JSON.stringify({ $extra: true })
        } else {
          result[name] = serialized
          // TODO Remove Extra if needed
        }
    ...
    async bindExtra (resourceId, ___extraValues = {}) {
      ...
      for (const extra of knownExtras) {
        ...
        const value = ___extraValues.value
        const needsUpdate = extra.value !== value
        if (needsUpdate) {
          await extra.update({ value })
        }
      }
      for (const [name, value] of Object.entries(___extraValues)) {
        const isKnown = name in knownExtras
        if (isKnown) {
          continue
        }
        console.log(resourceId, name, value)
        await ExtraModel.create({ resourceId, name, value })
      }
    },
