...
function resourceModel (sequelize, resourceName, options = {}) {
  ...
  const methods = {
    ...
    async restoreFromMeta (meta) {
      await this._restoreFromMetaLock
      this._restoreFromMetaLock = (async () => {
        try {
          ...
          this._restoreFromMetaLock = null
        }
      })
      await this._restoreFromMetaLock
    },
