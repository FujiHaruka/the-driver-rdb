  ...
  const methods = {
    ...
    async bindExtra (resourceId, ___extraValues = {}) {
      ...
      for (const [name, { type, value }] of Object.entries(___extraValues)) {
        ...
        const extra = await ExtraModel.findOne({ where: { resourceId, name } })
        if (extra) {
          await extra.update({ type, value })
        } else {
    ...
    async registerAttributes (attributeNames, options = {}) {
      ...
      for (const name of attributeNames) {
        Model.described = await Model.describe()
        if (META_ATTR_NAME_PREFIX.test(name)) {
          continue
        }
        ...
        const needsToAdd = !(rawName in Model.described)
        if (needsToAdd) {
          await this.withLock('addColumn', async () => {
            await sequelize.queryInterface.addColumn(Model.tableName, rawName, newRawAttributes[rawName])
          })
          await asleep(0)// next tick
        }
      }
      const needsSync = Object.keys(newRawAttributes).some((key) => {
        const isKnown = key in Model.rawAttributes
        return !isKnown
      })
      if (needsSync) {
    ...
    async prepare (options = {}) {
      ...
      await this.withLock('prepareLock', async () => {
        const knownAttributes = { ...defaultKnownAttributes }
        {
          const { registeredUnknownNames = [] } = Model
          const unknownNames = Object.keys(attributes)
            .filter((name) => name !== 'id')
            .filter((name) => !knownAttributes[name])
            .filter((name) => !registeredUnknownNames.includes(name))
          if (unknownNames.length > 0) {
            Model.registeredUnknownNames = [
              ...registeredUnknownNames,
              ...unknownNames,
            ].filter(uniqueFilter())
            const registered = await Model.registerAttributes(unknownNames)
            // TODO merge registeredUnknownNamesとknownAttributesを統合する
